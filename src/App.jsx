    ...
    <div className="space-y-5">
      ...
      <Card title="Coûts annexes (calcul auto)" accent="fuchsia" icon="⚙️">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div className={`bg-slate-900/50 rounded-lg px-4 py-3 border transition ${calc.cesarEnabled ? 'border-slate-700/40' : 'border-slate-800 opacity-60'}`}>
            <div className="flex justify-between items-center mb-1">
              <label className={`flex items-center gap-2 text-sm font-medium ${canWrite ? 'cursor-pointer' : ''}`}>
                <input type="checkbox" checked={calc.cesarEnabled} disabled={!canWrite} onChange={(e) => patchWeek(currentWeek.id, { cesar_enabled: e.target.checked })} className="w-4 h-4 rounded accent-fuchsia-500" />
                <span className={calc.cesarEnabled ? 'text-slate-300' : 'text-slate-500 line-through'}>Coût Cesar</span>
              </label>
              <span className={`text-lg font-bold ${calc.cesarEnabled ? 'text-fuchsia-300' : 'text-slate-500'}`}>{fmtEur(calc.cesarEnabled ? calc.cesarCost : calc.totalLeadsCost * 0.10)}</span>
            </div>
